if (typeof(LianMeng) != 'object')
{
    var LianMeng = {};
}
LianMeng.userAgent = navigator.userAgent.toLowerCase();

LianMeng.is_firefox = LianMeng.userAgent.indexOf('firefox')>=0?true:false;
LianMeng.is_opera = navigator.appName.indexOf('Opera')>=0?true:false;
LianMeng.is_safari = LianMeng.userAgent.indexOf('safari')>=0?true:false;
LianMeng.is_chrome = LianMeng.userAgent.indexOf('chrome')>=0?true:false;if(LianMeng.is_chrome)LianMeng.is_safari=false;
LianMeng.is_ie =navigator.appName=="Microsoft Internet Explorer"?true:false;
LianMeng.ie = (LianMeng.userAgent.indexOf('msie') != -1 && !LianMeng.is_opera) && LianMeng.userAgent.substr(LianMeng.userAgent.indexOf('msie') + 5, 3);

LianMeng.DOM = function(){
    return (document.compatMode && document.compatMode=="CSS1Compat")?document.documentElement:document.body;
}

LianMeng.$ = function(el){return document.getElementById(el);}
var $ = LianMeng.$;
 ...
 */
LianMeng.$import = function (jsfile,onLoadFun,onErrorFun)
{
    LianMeng._IMPORT_JSFILE = LianMeng._IMPORT_JSFILE || {};
    if (!LianMeng._IMPORT_JSFILE[jsfile])
    {
        LianMeng._IMPORT_JSFILE[jsfile] = 1;
 ...
 */
LianMeng.iniHtml = function(myobj){
    myobj = myobj||document;
    LianMeng._iniHtml_run = LianMeng._iniHtml_run||[];
    for (var _i=0;_i<LianMeng._iniHtml_run.length;_i++){
        if (typeof LianMeng._iniHtml_run[_i] == 'function'){
            try{
                LianMeng._iniHtml_run[_i](myobj);
            }catch(e){}
        }
    }

    //执行一次
    LianMeng._iniHtml_run_Once = LianMeng._iniHtml_run_Once||[];
    for (var _i=0;_i<LianMeng._iniHtml_run_Once.length;_i++){
        if (typeof LianMeng._iniHtml_run_Once[_i] == 'function'){
            try{
                LianMeng._iniHtml_run_Once[_i](myobj);
            }catch(e){}
        }
    }
    LianMeng._iniHtml_run_Once = [];
}

/**
 * 添加初始化HTML脚本的自定义方法
 */
LianMeng.iniHtmlAdd = function (fun){
    if (typeof fun == 'function'){
        LianMeng._iniHtml_run = LianMeng._iniHtml_run||[];
        LianMeng._iniHtml_run.push(fun);
    }
}
LianMeng.iniHtmlOnce = function (fun){
    if (typeof fun == 'function'){
        LianMeng._iniHtml_run_Once = LianMeng._iniHtml_run_Once||[];
        LianMeng._iniHtml_run_Once.push(fun);
    }
}

LianMeng.frameFrame = window.self;	//子窗口
LianMeng.parentFrame = window.self;	//父窗口
 ...
 */
LianMeng.Msg = function(msg, href, showtime){
    showtime = showtime||3;

    if (href){
        LianMeng.cookie.set('flash_message_',msg);
        document.location.href = href;
        return true;
    }
    if (typeof msg != 'string'){
        msg = LianMeng.cookie.get('flash_message_');
        if (msg)
            LianMeng.cookie.del('flash_message_');
    }
    if (msg){
        var runTime = 0;
        var tmpleft = 0;
        var width;
        var obj = LianMeng.$('LianMeng_show_msg_div_');

        if (LianMeng.is_ie && LianMeng.ie<7){
            //解决IE浮动问题
...
//                obj.style.left = '-'+tmpleft+'px';
        ...
        if (LianMeng._msgRunHidden){
            clearTimeout(LianMeng._msgRunHidden);
            width = obj.offsetWidth * 0.2;
            hidden(function(){
                LianMeng.Msg(msg,href,showtime);
            });
            return true;
        }
        if (!obj){
            obj = document.createElement('div');
            obj.style.cssText = 'z-index:111111;position:fixed;opacity:0.01;filter:alpha(opacity=1);top:0px;left:37%;padding:3px 12px;border:1px solid #ccc;overflow:hidden;background:#FF6600;width:26%;color:#fff;text-align:left;';
            obj.id = 'LianMeng_show_msg_div_';
...
//                obj.style.left = '0px';
                runTime = 0;
                LianMeng._msgRunHidden = setTimeout(hidden,showtime*500);
            }
        }
        setTimeout(show,200);
    }
}
LianMeng.iniHtmlOnce(function(){LianMeng.Msg()});
...
LianMeng.alert = function(alertset,w,h,title,handler) {
    LianMeng._set_frame();
    ...
    //try{LianMeng.parentFrame.ymPrompt.close();}catch(e){}
    if (!window.LianMeng.parentFrame.ymPrompt){
        ...
        if (alertset._type=='errorInfo'){
            window.LianMeng.parentFrame.ymPrompt.errorInfo(alertset);
        }else if(alertset._type=='succeedInfo'){
            window.LianMeng.parentFrame.ymPrompt.succeedInfo(alertset);
        }else if(alertset._type=='win'){
            window.LianMeng.parentFrame.ymPrompt.win(alertset);
        }else{
            window.LianMeng.parentFrame.ymPrompt.alert(alertset);
        }
        if (window.LianMeng.parentFrame)window.LianMeng.parentFrame.LianMeng.iniHtml(LianMeng.$('ym-window'));
    }
}

LianMeng.succeed = function(alertset,w,h,title,handler){
    if (typeof (alertset) != 'object'){
        alertset = {'message':(alertset||'')};
    }
    alertset.title = alertset.title || '操作成功';
    alertset._type = 'succeedInfo';
    LianMeng.alert(alertset,w,h,title,handler);
}
LianMeng.error = function(alertset,w,h,title,handler){
    if (typeof (alertset) != 'object'){
        alertset = {'message':(alertset||'')};
    }
...
LianMeng.win = function(alertset,w,h,title,handler){
    if (typeof (alertset) != 'object'){
        alertset = {'message':(alertset||'')};
    }
    alertset._type = 'win';
...
LianMeng.confirm = function (alertset,w,h,title,handler){
    LianMeng._set_frame();
    if (typeof (alertset) != 'object'){
    ...
    if (!LianMeng.parentFrame.ymPrompt){
        ...
        try{LianMeng.parentFrame.ymPrompt.close();}catch(e){}
        LianMeng.parentFrame.ymPrompt.confirmInfo(alertset,w,h,title,handler);
        LianMeng.parentFrame.LianMeng.iniHtml(LianMeng.$('ym-window'));
    }
}


LianMeng.closewin = function(type,autoclose){
    LianMeng._set_frame();
    try{LianMeng.parentFrame.ymPrompt.doHandler(type,autoclose);}catch(e){}
}


LianMeng.urlpath = '/';
LianMeng.ask_del = function(id,delurl,msg){
    msg=msg||'您确认要删除此数据？';
    LianMeng.confirm(msg,400,null,'删除确认',function(tp){if (tp=='ok') goUrl( LianMeng.urlpath + delurl +'/'+id,'hiddenFrame')});
}


LianMeng.show_loading = function(){
    var obj = LianMeng.$('_the_loading_div');
    if (!obj)
    {
        obj = document.createElement('div');
        obj.id = '_the_loading_div';
        obj.style.cssText = 'z-index:20000;position:fixed;_position:absolute;top:0;left:0;text-align:center;width:100%;color:#111;font-size:14px;top:expression((eval(document.compatMode&&document.compatMode=="CSS1Compat")?documentElement.scrollTop:document.body.scrollTop)+LianMeng._show_loading_ie6_top_);';
        obj.innerHTML = '<span class="loading_div">正在加载，请稍等...</span>';
        document.body.appendChild(obj);
    }
    var top = LianMeng.DOM().clientHeight/2-30;
    LianMeng._show_loading_ie6_top_ = top;
    obj.style.top = top+'px';
    obj.style.display = '';
}

LianMeng.hidden_loading = function(){
    var obj = LianMeng.$('_the_loading_div');
    if (obj)
    {
...
LianMeng._set_frame = function(){
    try {
        if( typeof(window.parent.LianMeng)=='object' )
        {
            LianMeng.parentFrame = window.parent;
        }
    }
    catch(e)
    {
    }
    LianMeng.parentFrame.LianMeng.frameFrame = window.self;
}
 ...
 */
LianMeng.popMsg = function(msg,href,time){
    if (href){
        LianMeng.cookie.set('flash_pop_message_',msg);
        document.location.href = href;
    ...
    time = time||5;
    if (typeof msg != 'string'){
        msg = LianMeng.cookie.get('flash_pop_message_');
        if (msg)
            LianMeng.cookie.del('flash_pop_message_');
    }
    if (!msg)return false;

    if (LianMeng._pop_msg_timeout_show){
        clearTimeout(LianMeng._pop_msg_timeout_show);
        LianMeng._pop_msg_timeout_show = null;
    }
    if (LianMeng._pop_msg_timeout_hidden){
        clearTimeout(LianMeng._pop_msg_timeout_hidden);
        LianMeng._pop_msg_timeout_hidden = null;
    }
    var displayOpacity = function(id, n) {
        ...
        if(n >= 0) {
            n -= 10;
            LianMeng.$(id).style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + n + ')';
            LianMeng.$(id).style.opacity = n / 100;
            LianMeng._pop_msg_timeout_show = setTimeout(function(){displayOpacity(id, n);}, 50);
        } else {
            LianMeng.$(id).style.display = 'none';
            LianMeng.$(id).style.filter = '';
            LianMeng.$(id).style.opacity = '';
        }
    }
    var showmsgwin = function(b, e, a, t) {
        var step = (b - e) / 10;
        var msgWinObj = LianMeng.$('msgwin');
        var newp = (parseInt(msgWinObj.style.top) - step);
        if(newp > e) {
            msgWinObj.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + a + ')';
            msgWinObj.style.opacity = a / 100;
            msgWinObj.style.top = newp + 'px';
            LianMeng._pop_msg_timeout_hidden = setTimeout(function () {showmsgwin(b, e, a += 10, t)}, 10);
        } else {
            msgWinObj.style.filter = '';
            msgWinObj.style.opacity = '';
            LianMeng._pop_msg_timeout_show = setTimeout(function(){displayOpacity('msgwin', 100);}, t);
        }
    }
    var msgWinObj = LianMeng.$('msgwin');
    if (!msgWinObj){
 ...
 * @parem ask 内容
 */
LianMeng.ask_todo = function(url,ask)
{
    LianMeng.confirm(
        {
            'message':ask||'请确认',
            ...
            'handler':function(el){
                ...
                LianMeng.run_ajax(url);
            }
        }
    );
}

LianMeng.run_ajax = function(url)
{
    if (!LianMeng._AJAX_RUNNING)LianMeng._AJAX_RUNNING = {};
    if (LianMeng._AJAX_RUNNING[url])
    {
        LianMeng.Msg('页面正在执行，请稍等...');
        return;
    }
    LianMeng._AJAX_RUNNING[url] = 1;
    LianMeng.show_loading();

    var ajax = new LianMeng.ajax(url);
    ajax.method = 'POST';
    ajax.onError = function()
    {
        LianMeng._AJAX_RUNNING[url] = null;
        LianMeng.hidden_loading();

        if (this.status == 404)
        {
            LianMeng.error('指定的页面不存在');
        }
        else
        {
            LianMeng.error('页面执行失败，请重试。');
        }
        return false;
    }
    ...
    ajax.onCompletion = function()
    {
        ...
        {
            LianMeng.error('数据异常，请重试或联系管理员。');
            return false;
        }

        if (this.responseJSON['code']==1)
        {
            LianMeng.Msg(this.responseJSON['msg'],document.location);
        }
        else if (this.responseJSON['code']<0)
        {
            LianMeng.error(this.responseJSON['msg']);
        }
        else{
            LianMeng.alert(this.responseJSON['msg']);
        }
    }
...
LianMeng.tag = function(thisid,titlePrefix,mainPrefix,searchID,classname){
    var mainobj;
    if (searchID)
    {
        if (typeof searchID == 'object')
        {
            ...
            mainobj = LianMeng.$(searchID) || document.body;
        }
    }else{
    ...
    for (var i=0;i<el.length;i++)
    {
        if (el[i].id)
        {
            if (el[i].id.substr(0,titlePrefix.length+1) == titlePrefix+'_')
            {
                ...
                var mainobj = LianMeng.$(mainPrefix + thisTitleId);
                if (el[i].id != thisid)
                {
                    el[i].className = '';
                    if (mainobj)
 ...
 * @return array
 */
LianMeng.position = function(obj) {
    var r = new Array();
    r['x'] = obj.offsetLeft;
    r['y'] = obj.offsetTop;
...
LianMeng.ajax = function(file) {
    ...
    this.createAJAX = function() {
        ...
        if (this.streaming && LianMeng.is_ie && LianMeng.ie<9)
        {
    ...
    this.createIframeAjax = function(){
        ...
        if (!this.iframeobj){
            var obj = LianMeng.$('_ajax_iframediv_'+host.replace(/\./g,'_'));
            if (!obj){
                ...
                //防止IE在没有加载完全后出错
                document.body.insertBefore(obj,LianMeng.$('header_div'));
            }
            this.iframeobj = obj;
        }
    ...
    {
        // 只有IE
        if (!LianMeng.is_ie)return false;
        var self = this;
        // 构造一个虚拟的xmlhttp对象
        this.xmlhttp = {
            responseXML : null,
            ...
            open : function(method,url){
                ...
                url = newurl.join('#');
                this.url = url;
                this.ifrDiv.innerHTML = '<iframe id="LianMeng_ajax_iframe" name="LianMeng_ajax_iframe"></iframe><form id="LianMeng_ajax_fram" target="LianMeng_ajax_iframe" action="'+url+'" method="POST"></form>';
                var objframe = this.obj.getElementById('LianMeng_ajax_iframe');
            ...
            send : function(data){
                ...
                {
                    //GET方式最简单了
                    this.obj.getElementById('LianMeng_ajax_iframe').src = this.url;
                }
                else
                {
                    ...
                    this.obj.getElementById('LianMeng_ajax_fram').innerHTML = html;
                    this.obj.getElementById('LianMeng_ajax_fram').submit();
                    //构造出一个表单提交
                }
            },
    ...
    this.createFlashAjax = function(){
        if (!this.flashobj){
            var obj = LianMeng.$('_ajax_fdiv');
            if (!obj){
                obj = document.createElement('DIV');
                obj.id='_ajax_fdiv';
                //Window的firefox里必须在可见区域，且不容许隐藏
                obj.style.cssText = 'position:fixed;_position:absolute;top:100px;left:0px;opacity:0.01;';
                //防止IE在没有加载完全后出错
                document.body.insertBefore(obj,LianMeng.$('header_div'));
            }
            if (!LianMeng.get_flash('_ajax_fdiv_flash')){
                LianMeng.flash('_ajax_fdiv',LianMeng.Url.Statics+'/swf/ajaxcdr.swf',1,1,null,null,null,{'allowscriptaccess':'always','swliveconnect':'true','bgcolor':'#ffffff'});
            }
            this.flashobj = LianMeng.get_flash('_ajax_fdiv_flash');
        }
        var time = new Date().getTime()+'_'+ Math.round(Math.random()*10000);
        if (this.flashobj.is_loading==true){
            ...
            obj.style.cssText = 'position:fixed;_position:absolute;top:100px;left:0px;opacity:0.01;';
            document.body.insertBefore(obj,LianMeng.$('header_div'));
            LianMeng.flash('_ajax_fdiv_'+time,LianMeng.Url.Statics+'/swf/ajaxcdr.swf',1,1,null,null,null,{'allowscriptaccess':'always','swliveconnect':'true'});
            this.flashobj = LianMeng.get_flash('_ajax_fdiv_'+time+'_flash');
        }
        var self = this;
        if (typeof LianMeng._flashAjaxCallback != 'object'){
            LianMeng._flashAjaxCallback = [];
        }
        this.xmlhttp = new function(){
            var method = null;
            var url = null;
                ...
                var loadhttp = function(){
                    setTimeout(function(){
                        ...
                        if (self.flashobj.XmlHttp){
                            self.flashobj.XmlHttp(url , 'LianMeng._flashAjaxCallback["'+randid+'"]' , method , urlstring, 'application/x-www-form-urlencoded');
                        }else{
                            //9秒钟（300次尝试获取数据）
                            if (tryNum<300){
                                loadhttp();
                                ...
                                self.xmlhttp.responseText = '';
                                self.xmlhttp.onreadystatechange();
                                delete LianMeng._flashAjaxCallback[time];
                            }
                        }
                    },30);
                }
        ...
        LianMeng._flashAjaxCallback[time] = function(){
            self.xmlhttp.readyState = 2;
            self.xmlhttp.onreadystatechange();
            self.xmlhttp.readyState = 3;
            self.xmlhttp.onreadystatechange();
...
LianMeng.suggest = function (obj){
    ...
    obj.onkeydown = function(e){
        ...
        if (e.keyCode==40||e.keyCode==38){
            ...
            // 回车
            var obj_div = LianMeng.$('_suggest_div');
    ...
    obj.onkeyup = function(e){
        ...
        else if(e.keyCode==13||e.keyCode==27)
        {
            ...
            self.timeout_run = setTimeout(function(){
                ...
                if(self.url)
                {
                    var ajax = new LianMeng.ajax(self.url);
    ...
    obj.onfocus = function(){
        ...
        if (LianMeng._suggest_blur)clearTimeout(LianMeng._suggest_blur);
    }
    obj.onblur = function(){
        var obj_div = LianMeng.$('_suggest_div');
        if (obj_div){
        ...
        LianMeng._suggest_blur = setTimeout(function(){
...
//        }
        ...
        for(var i=0;i<lis.length;i++){
            ...
            if ( lis[i].getAttribute('value')==obj.value )
            {
                ...
                var objdiv2 = LianMeng.$('_suggest_div_scroll_div');
    ...
    {
        return '<div style="padding-left:5px;color:blue">'
            + key
            +'&nbsp;</div></td><td style="border: none;padding: 0px; margin: 0px;"><div style="padding-right:10px;">= '+value+'</div>';
    }
}
 ...
 * @param callBack 成功回调，仅在goto_url为空的情况下有效
 */
LianMeng.form_post = function (obj,goto_url,options, callBack)
{
    if (obj.is_postting==1)
    {
        LianMeng.Msg('数据正在提交，请稍等...');
        return false;
    }
    LianMeng.show_loading();

    obj.is_postting = 1;
    var ajax = new LianMeng.ajax(obj.action||document.location.href);
    ...
    ajax.onCompletion = function()
    {
        ...
        if (this.responseJSON['code']==1)
        {
            LianMeng.Msg(this.responseJSON['msg'],goto_url);
            if (!goto_url && typeof callBack === 'function') {
                callBack(this.responseJSON['data']);
            }
        }
 ...
 * 滑动门效果打开或关闭div
 */
LianMeng.resizeDiv = function(obj,handler){
    var objid = obj.id;
    var objnoid = false;
    if (!objid){
    ...
    LianMeng._resizeDiv_runNum=LianMeng._resizeDiv_runNum||{};
    if (LianMeng._resizeDiv_runNum[objid]>0){
        return false;
    }
    LianMeng._resizeDiv_runNum[objid] = 0;
    ...
    var changeDiv = function(){
        LianMeng._resizeDiv_runNum[objid]++;
        var runNum = LianMeng._resizeDiv_runNum[objid];
        ...
        if (runNum<=5){
            ...
            LianMeng._resizeDiv_runNum[objid] = 0;
            delete LianMeng._resizeDiv_runNum[objid];
            if (handler){
                try{handler();}catch(e){}
            }
            if (objnoid){
 ...
 * @return
 */
LianMeng.flash = function(id,url,w,h,p,rightmenu,flashvars,other,ourflash){
 ...
 */
LianMeng.flash_version = function() {
...
LianMeng.get_flash = function(objname){
    if (document[objname]){
 ...
 */
LianMeng.addfavorite = function(url,title)
{
    ...
    else if (window.sidebar)
    {
        ...
        LianMeng.Msg('您的浏览器不支持此功能，请手动加入，谢谢。');
    }
}
 ...
 *
 *    // 获取
 *    LianMeng.localStorage.getItem('test');
 *    // 设置
 *    LianMeng.localStorage.setItem('test','123');
 *    // 删除
 *    LianMeng.localStorage.removeItem('test');
 *
 */
LianMeng.localStorage = {
    getItem : function(key)
    {
        if( LianMeng.localStorage.issup() )
        {
            return localStorage.getItem(key);
        }
        else
        {
            return LianMeng.cookie.get('_'+key);
        }
    },
    setItem : function(key,value)
    {
        ...
        {
            return LianMeng.cookie.set('_'+key,value,999,'/');
        }
    },
    removeItem : function(key)
    {
        ...
        {
            return LianMeng.cookie.del('_'+key);
        }
    },
    issup : function(){
...
window.scroll = function(x,y){
    if (LianMeng._scroll_runNum_>0){
        return true;
    }
    LianMeng._scroll_runNum_ = 0;
    var runNum = LianMeng._scroll_runNum_;
    var scrollX = LianMeng.DOM.scrollLeft || document.body.scrollLeft;
    var scrollY = LianMeng.DOM.scrollTop || document.body.scrollTop;
    var tmpX = scrollX-x;
    var tmpY = scrollY-y;
    var scroll = function(){
        runNum++;
