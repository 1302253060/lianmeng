...
function checkIP($ip){
    ...
    foreach ($inter_ip as $value) {
        if( $ip_num>=$value[0] && $ip_num<=$value[1] ){
            return false;
        }
    }
    return true;
...
{
    //忽略抓取时间限制
    $info = parse_url($uri, PHP_URL_PATH);

    if( $info && !empty( $info['host'] )&&strlen($info['host']) <100) {
        $ip = gethostbyname( $info['host'] );
    }else{
        echo "{'url': error','tip':'远程图片抓取失败！','srcUrl':'" . $uri . "'}";
        return ;
    }
    if(  checkIP( $ip ) == false ){
        echo "{'url': error','tip':'远程图片抓取失败！','srcUrl':'" . $uri . "'}";
