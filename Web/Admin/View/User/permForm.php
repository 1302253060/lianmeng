...
<div id="perm_checkbox_div">
    ...
    if (!$perm)
    {
        $perm = new \Common\Helper\Permission(array());
    }
    $my_perm = \Common\Helper\Session::instance()->member()->perm();
    ...
    foreach ($perm_config as $key=>$value)
    {
        ...
        if ($value['perm']) foreach ($value['perm'] as $k=>$v) {
            if (preg_match('#^\d+$#', $k)) {
                # 根据配置修改前置HTML
                $outhtml_left = ($have_group_perm ? '</td></tr>' : '')
                    . '<tr><td width="120" class="td1" align="right"><label>'
                    . trim($v,'-')
                    . '<input type="checkbox"  onclick="select_group_perm(this.parentNode.parentNode.parentNode,this.checked)" title="全选" />'
                    . '</label></td><td class="td2"><ul class="ul perm_ul">';
                # 重置
                $have_group_perm = false;
                continue;
            }
            ...
            $tmpstr .= '<li><label title="'. $title .'">'.\Common\Helper\Form::checkbox($field_key,'1',$checked,$input_att).$v['name'].'</label></li>';
        }
        $tmpstr .= '</ul></td></tr>';
        if ($have_perm)
        {
            ...
            <table class="mainTable" style="margin-bottom:6px;">
                <tr>
                    <td<?php if ($have_group)echo ' colspan="2"';?> style="text-align: center; font-weight: bold;"  ><label><?php echo $value['name'];?><input type="checkbox" onclick="select_group_perm(this.parentNode.parentNode.parentNode.parentNode,this.checked)" title="全选" /></label></td>
                </tr>
                <tr>
                    <?php echo $tmpstr;?>
                </tr>
